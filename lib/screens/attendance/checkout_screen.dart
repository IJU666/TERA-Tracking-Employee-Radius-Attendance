...
void initState() {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) async {
    ...
    try {
      ...
      debugPrint('🟢 4. Selesai checkCheckoutStatus, _isLoadingStatus=$_isLoadingStatus');
    } catch (e, st) {
      debugPrint('🔴 ERROR saat init CheckoutScreen: $e');
      debugPrint('$st');
      if (mounted) {
        setState(() {
          _isLoadingStatus = false; // pastikan tidak stuck loading
...
Future<void> _checkCheckoutStatus() async {
  ...
  Future<void> _submitCheckout() async {
    ...
    try {
      ...
      // Menyimpan data checkout dan mengunci hari ini menggunakan 'tanggalTerakhirCheckout'
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update({
        'statusHariIni': 'Pulang', // Merubah status menjadi Pulang agar terbaca di HomeScreen
        'jamPulang': timeStr,      // Mencatat jam pulang untuk indikator di HomeScreen
        'tanggalTerakhirCheckout': todayStr, // Penanda kunci checkout harian
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Checkout berhasil di posisi $_officeName! Sampai jumpa besok.'),
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Info status radius / info sudah checkout hari ini
                        if (_alreadyCheckout)
                          Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              if (_isLoadingLocation)
                                ...
                                InkWell(
                                  onTap: _alreadyCheckout ? null : _refreshLocation,
                                  child: Text(
                                    'Refresh',
                                    style: TextStyle(
                                      color: _alreadyCheckout ? Colors.grey : Colors.blue, 
                                      fontWeight: FontWeight.bold,
                                    ),
                                  ),
                                ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: isButtonEnabled ? _submitCheckout : null,
                            ...
                            icon: _isSubmitting 
                                ...
                                : Icon(_alreadyCheckout ? Icons.check : Icons.fingerprint, color: Colors.white),
                            label: Text(
                              _isSubmitting 
                                  ? 'Memproses...' 
                                  : (_alreadyCheckout ? 'Sudah Checkout Hari Ini' : 'Konfirmasi Checkout'),
